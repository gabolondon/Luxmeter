...
void setup(){
  ...
  if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { // Address 0x3D for 128x64
    Serial.println(F("No se pudo conectar al display con esa dirección"));
  }
  display.display();
  delay(2000);
...
void loop() {
  ...
  Serial.println(" lx");
  //if(!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { // Address 0x3D for 128x64
    //Serial.println(F("display desconectado"));
  //}else{
    display.setTextSize(1);             // Normal 1:1 pixel scale
    display.setTextColor(SSD1306_WHITE);        // Draw white text
    display.setCursor(0,0);             // Start at top-left corner
    display.println(F("Medidor de Lumens")); 
    display.println(F("Intensidad: "));  
